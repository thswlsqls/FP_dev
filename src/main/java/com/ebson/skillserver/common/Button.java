...
@Data
public class Button {
    private String label; // 버튼에 적히는 문구입니다. 버튼 14자(가로배열 2개 8자). 썸네일이 1:1이면 버튼이 가로배열 됩니다.
    private String action; // 버튼 클릭시 수행될 작업입니다.
    private String webLinkUrl; // 웹 브라우저를 열고 webLinkUrl 의 주소로 이동합니다. URL
    private String messageText; // message: 사용자의 발화로 messageText를 내보냅니다. (바로가기 응답의 메세지 연결 기능과 동일) block: 블록 연결시 사용자의 발화로 노출됩니다.
