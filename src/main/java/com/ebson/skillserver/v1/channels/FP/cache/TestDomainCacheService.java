...
public class TestDomainCacheService {
    ...
    public TestDomain getTestDomainByIdAndName(Long id, String name) {
        // 이는 캐시 키를 "TestDomain:[id]:[name]" 형식으로 생성하도록 합니다. 여기서 #id와 #name은 메서드의 파라미터 id와 name을 참조합니다.
    ...
    public TestDomain getTestDomain(TestDomain testDomain){
        // 이는 캐시 키를 "TestDomain:[testDomain.id]:[testDomain.name]" 형식으로 생성하도록 합니다. 여기서 #testDomain.id와 #testDomain.name은 메서드의 파라미터 testDomain 객체를 참조합니다.
